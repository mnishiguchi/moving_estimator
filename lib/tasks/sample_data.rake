...
def make_movings
  users = User.take(6)
  users.each do |user|
    (1..10).to_a.sample.to_i.times do
      title = "from #{Faker::Address.city} to #{Faker::Address.city}"
      description = Faker::Lorem.sentence(3)
      moving = user.movings.create!(title: title, description: description)
      (1..50).to_a.sample.to_i.times do
        name        = Faker::Commerce.product_name
        volume      = Faker::Number.decimal(1)
        quantity    = Faker::Number.between(1, 10)
        room        = ["living room", "dining room", "bedroom", "kitchen", "bathroom"].sample
        category    = %w(ocean air local disposal).sample
        description = Faker::Lorem.sentence(3)
